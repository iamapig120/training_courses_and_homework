...
<body>
    <div id="topWindow">
        <canvas id="loginAnime" width="864" height="364"></canvas>
        <img id="loginBG" src="content/img/loginBG.png" alt="SlimChat登录" />
        <span id="mimWinBtn">-</span>
        <span id="closeWinBtn">×</span>
    </div>
    <div id="mainWindow">
        <img id="avatar" src="content/test/head.jpg" alt="头像" />
        <div id="mainMiddle">
            <input id="userName" class="inputForm" type="text" autocomplete="off" placeholder="账号/手机号/邮箱">
            <input id="password" class="inputForm" type="password" autocomplete="off" placeholder="密码">
            <div id="checkboxForms">
                <label>
                    <input id="savePassword" name="savePassword" type="checkbox">记住密码</label>
                <label>
                    <input id="autoLogin" name="autoLogin" type="checkbox">自动登录</label>
            </div>
        </div>
        <div id="mainRight">
            <a href="http://182.254.136.143:2333/register.html" class="link" target="_blank">注册账号</a>
            <a href="http://182.254.136.143:2333/findpassword.html" class="link" target="_blank">找回密码</a>
        </div>
...
<script>
    ...
    (function () {
        ...
        const LOGIN_POST_URL = "login-post.php";
        //
        // //最小化
                    ...
                    success: jsonObj => {
                        if (jsonObj.state === "success") {
                            ...
                            setTimeout(() => {
                                ...
                                document.getElementById("loginBtn").innerText = "登录";
                            }, 500);
                        }
                        console.log(jsonObj);
                    }
                }); // 请求json文件
                //ipc.send('login-action', usr, pass);
                //修改样式
                document.getElementById("mainWindow").className = "logining";
                document.getElementById("loginBtn").innerText = "取消";
            } else if (!usr) {
        ...
        function showWindow(time) {
            return new Promise((resolve) => {
                ...
                function reDraw(timestamp) {
                    ...
                    let timeGonePersent = (timestamp - startTime) / time;
                    if (timeGonePersent > 1) timeGonePersent = 1;
                    document.body.style.opacity = timeGonePersent;
                    if (timeGonePersent == 1) {
                        resolve();
                    } else {
                        window.requestAnimationFrame(reDraw);
                    }
        ...
        function hideWindow(time) {
            return new Promise((resolve) => {
                ...
                function reDraw(timestamp) {
                    ...
                    let timeGonePersent = (timestamp - startTime) / time;
                    if (timeGonePersent > 1) timeGonePersent = 1;
                    document.body.style.opacity = 1 - timeGonePersent;
                    if (timeGonePersent == 1) {
                        resolve();
                    } else {
                        window.requestAnimationFrame(reDraw);
        ...
        //绑定用户名和密码框的鼠标经过事件
        function bindInputMouseoverEvents() {
            let mainMiddle = document.getElementById("mainMiddle");
            let userName = document.getElementById("userName");
            userName.addEventListener("mouseover", () => {
                mainMiddle.className = "select1";
            });
            userName.addEventListener("mouseout", () => {
                mainMiddle.className = "";
            });
            let password = document.getElementById("password");
            password.addEventListener("mouseover", () => {
                mainMiddle.className = "select2";
            });
        ...
        //绑定保存账号密码选项
        Array.from(
            document.getElementById("checkboxForms").getElementsByTagName("label")).forEach(
            (e) => {
                e.getElementsByTagName("input")[0].addEventListener("change", function () {
                    if (this.checked) {
                        e.className = "select";
                    } else {
                        e.className = "";
        ...
        document.getElementById("checkboxForms").getElementsByTagName("label")[1].getElementsByTagName("input")[0].addEventListener("change", function () {
            if (this.checked) {
                document.getElementById("checkboxForms").getElementsByTagName("label")[0].getElementsByTagName("input")[0].checked = true;
                document.getElementById("checkboxForms").getElementsByTagName("label")[0].className = "select";
            }
        });
        document.getElementById("checkboxForms").getElementsByTagName("label")[0].getElementsByTagName("input")[0].addEventListener("change", function () {
            if (!this.checked) {
                document.getElementById("checkboxForms").getElementsByTagName("label")[1].getElementsByTagName("input")[0].checked = false;
                document.getElementById("checkboxForms").getElementsByTagName("label")[1].className = "";
            }
        });
        //绑定账号密码输入框的回车事件

        Array.from(document.getElementsByClassName("inputForm")).forEach((o) => {
        ...
        document.onreadystatechange = () => {
            (function () {
                if (document.readyState == "interactive") {
                    showWindow(500).then(function () {
                        bindInputMouseoverEvents();
                    });
                }
            })();
        }
